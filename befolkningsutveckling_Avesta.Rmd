...
* Nedgången i `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)`s folkmängd år 2023, jämfört med året innan, beror i första hand på att såväl det inrikes som det utrikes flyttnettot minskade jämfört med året innan.
...
* Det var framförallt personer under 30 år som flyttade från `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` till andra delar av Sverige under 2023 (netto).
...
Om fokus enbart läggs på åldersgruppen 20-29 år, blir det tydligt att det i praktiken alltid flyttar fler unga vuxna från än till kommunen inrikes. Historiskt varierar det inrikes flyttnettot i åldersgruppen mellan åren, vilket exempelvis kan bero på att olika årskullar av barn är olika stora. År 2023 flyttade 41 personer fler i åldersgruppen 20-29 år från än till kommunen inrikes. Undantaget från trenden var 2015, när ett stort antal yngre flyttade till kommunen.
...
* `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` befolkning prognostiseras att minska med närmare `r format(abs(plyr::round_any(befprognos_df %>% filter(alder_grupp == "Totalt",region=="Avesta") %>% .$Folkmängd,100)),big.mark=" ")` personer fram till 2040, vilket är tydligt fler än vid den föregående prognosen. I den prognosen, som släpptes 2022, prognostiserades en minskning på under 700 personer (se [SCB](https://www.statistikdatabasen.scb.se/pxweb/sv/ssd/START__BE__BE0401__BE0401B/BefProgRegFakN22/table/tableViewLayout1/){target="_blank"}).

* Den största minskningen sker i gruppen som brukar benämnas arbetsför ålder, 20-64 år,  som prognostiseras minska med drygt `r format(abs(plyr::round_any(befprognos_df %>% filter(alder_grupp == "20-64 år",region=="Avesta") %>% .$Folkmängd,100)),big.mark=" ")` personer.
