...
## Utrikes flyttnetto
Som vi berörde i föregående avsnitt flyttade många till `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` under åren runt flyktingkrisen, vilket till viss del kan förklaras av att invandringen då var stor. I slutet av 1990-talet var invandringen låg eller till och med negativ under flera år (fler utvandrade än invandrade), men därefter skedde en gradvis ökning innan den stora toppen 2016. Därefter har invandringen minskat avsevärt. Under 2023 var det utrikes flyttnettot `r flytt_df %>% filter(år==max(år),region == region_val, variabel == "Invandringsöverskott") %>% group_by(region) %>% summarize(varde=sum(varde)) %>% .$varde` personer, det lägsta nettot sedan åtminstone 1997. Detta kan sannolikt delvis förklaras av de avregistreringar av utvandrade som Skattemyndigheten genomförde under 2023 (se informationsruta ovan), men en viktig anledningen är naturligtvis även en stramare nationell migrationspolitik.
...
### Uppdelat på ålder
Ett tydligt mönster är att det i första hand är barn och unga under 20 år och personer mellan 30 år och 50 år som har flyttat till Orsa på senare år (netto). En förklaring till detta kan vara att det är barnfamiljer som flyttar till kommunen. I äldre åldersgrupper tenderar det inrikes flyttnettot att växla mellan positiva och negativa år. 
...
Om vi enbart fokuserar på åldersgruppen 20-29 år, blir det dock tydligt att det i normala fall flyttar fler unga vuxna från än till kommunen inrikes. Historiskt varierar det inrikes flyttnettot i åldersgruppen mellan åren, vilket exempelvis kan bero på att olika årskullar av barn är olika stora. År 2023 flyttade som `r abs(flytt_aldersgrupper_df %>% filter(år == max(år),variabel == "Inrikes flyttningsöverskott",region == region_val,alder_grupper == "20-29 år") %>% .$varde)` personer fler i åldersgruppen 20-29 år från än till kommunen inrikes, vilket var första gången sedan 2016 som kommunen inte hade ett negativt flyttnetto i denna åldersgrupp.
