...
I diagrammet nedan visas befolkningsutvecklingen i `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` fram till 2033 uppdelat på åldersgrupper, enligt SCB:s prognos från juni 2024 (se [SCB](https://www.statistikdatabasen.scb.se/pxweb/sv/ssd/START__BE__BE0401__BE0401A/BefProgRegFakN/){target="_blank"}), vilket är den senaste som myndigheten publicerat offentligt på kommunal nivå. Notera att diagrammet kan skilja sig åt jämfört med motsvarande diagram i förra årets rapport. Detta eftersom befolkningen 2033 enligt SCBs prognos jämförs med den faktiska befolkningen vid slutet av 2024 i diagrammet nedan. I förra årets rapport jämfördes prognosen för 2033 med den faktiska befolkningen vid slutet av 2023.

* `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)`s befolkning prognostiseras att minska med drygt `r format(abs(plyr::round_any(befprognos_df %>% filter(alder_grupp == "Totalt",region== region_val) %>% .$Folkmängd,50)),big.mark=" ")` personer till 2033, jämfört med 2024. I SCB:s föregående prognos, som släpptes 2022, prognostiserades en minskning på ungefär 670 personer fram till 2033, jämfört med 2021 (se [SCB](https://www.statistikdatabasen.scb.se/pxweb/sv/ssd/START__BE__BE0401__BE0401B/BefProgRegFakN22/table/tableViewLayout1/){target="_blank"}).
