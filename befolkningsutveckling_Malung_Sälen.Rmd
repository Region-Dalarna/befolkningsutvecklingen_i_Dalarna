...
* Det föddes något fler barn i `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` 2023, jämfört med 2022, men samtidigt var antalet avlidna i kommunen klart högre än åren innan. 
...
I likhet med flera andra mindre kommuner i Dalarna har befolkningen i `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` minskat  sedan slutet på 1960-talet, från ca `r format(plyr::round_any(befolkning_df %>% filter(år == min(år),region == region_val ,variabel == "Folkmängd") %>% .$varde,100),big.mark=" ") ` invånare  år `r min(befolkning_df$år)` till ungefär `r format(plyr::round_any(befolkning_df %>% filter(år == max(år),region == region_val, variabel == "Folkmängd") %>% .$varde,100),big.mark=" ") ` invånare år `r max(befolkning_df$år)`. För `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` har dock utvecklingen sett annorlunda ut på senare år, då befolkningen, med undantag för 2018 , har ökat varje år sedan 2014.

År `r max(befolkning_df$år)` ökade folkmängden med (`r befolkning_df %>% filter(år == "2023",region == region_val, variabel == "Folkökning") %>% .$varde ` personer, vilket var klart färre än åren innan. 
...
En trend är att det dör färre personer i `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` idag än det gjorde längre tillbaka i tiden. Normalt dör mellan 110 och 140 personer årligen i kommunen, att jämföra med närmare 200 personer under några år i början av 1990-talet. Detta kan delvis förklaras av att färre idag bor i kommunen, men den främsta anledningen är framsteg inom det medicinska området. Den förväntade livslängden har ökat över tid.

Även när det gäller antalet födslar är den långsiktiga trenden negativ. Under det sena 1980-talet och det tidigt 1990-talet föddes ofta runt barn i `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` per år, men en tydlig negativ trend inleddes i samband med finanskrisen i början på 1990-talet. Under 2000-talet har antalet födda legat på en relativt stabil nivå, även om variationen kan vara stor från år till år. `r max(fodda_doda_df$år)` föddes `r fodda_doda_df %>% filter(år == max(år),region == region_val,variabel == "födda") %>% .$varde` barn, viket är något fler än åren innan.
...
## Utrikes flyttnetto
Som vi berörde i föregående avsnitt har relativt många flyttat till `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` under åren efter flyktingkrisen, vilket under framförallt åren runt flyktingkrisen kan förklaras med att invandringen har varit hög. I slutet av 1990-talet var invandringen låg eller till och med negativ under flera år (fler utvandrade än invandrade), men därefter skedde en gradvis ökning innan den stora toppen 2016. Därefter har invandringen minskat. Under 2023 var det utrikes flyttnettot `r flytt_df %>% filter(år==max(år),region == region_val, variabel == "Invandringsöverskott") %>% group_by(region) %>% summarize(varde=sum(varde)) %>% .$varde` personer, vilket var klart lägre än året innan. Detta beror sannolikt på en stramare nationell migrationspolitik.
...
Bland de åldersgrupper som redovisas i diagrammet nedan skedde den största utflyttningen år `r max(flytt_aldersgrupper_df$år)` i gruppen som är under 20-29 år, vilket dock var en något lägre utflyttning än året innan. Bland barn och unga (0-19 år) var inflyttningen (netto) relativt stor `r max(flytt_aldersgrupper_df$år)`, medan det omvända gällde året innan.

```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.height=5, fig.width=8, fig.align='center'}
gg_flytt_alder$`Inrikes_flyttnetto_alder_Malung-Sälen`
```

Om vi enbart fokuserar på åldersgruppen 20-29 år, blir det tydligt att det i normala fall flyttar fler unga vuxna från än till kommunen inrikes. Historiskt varierar det inrikes flyttnettot i åldersgruppen mellan åren, vilket exempelvis kan bero på att olika årskullar av barn är olika stora. År 2023 flyttade som `r abs(flytt_aldersgrupper_df %>% filter(år == max(år),variabel == "Inrikes flyttningsöverskott",region == region_val,alder_grupper == "20-29 år") %>% .$varde)` personer fler i åldersgruppen 20-29 år från än till kommunen inrikes, vilket var något färre än året innan. 
...
* `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` befolkning prognostiseras att minska med drygt `r format(abs(plyr::round_any(befprognos_df %>% filter(alder_grupp == "Totalt",region==region_val) %>% .$Folkmängd,10)),big.mark=" ")` personer fram till 2040, vilket är färre än vid föregående prognos. I den prognosen, som släpptes 2022, prognostiserades en minskning på drygt 400 personer (se [SCB](https://www.statistikdatabasen.scb.se/pxweb/sv/ssd/START__BE__BE0401__BE0401B/BefProgRegFakN22/table/tableViewLayout1/){target="_blank"}) fram till 2040.
...
```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.height=5, fig.width=8, fig.align='center'}
gg_befprognos$`befolkningsforandring_Malung-Sälen_2040`
```
