...
I likhet med flera andra mindre kommuner i Dalarna har befolkningen i `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` minskat  sett över längre tid, från ca `r format(plyr::round_any(befolkning_df %>% filter(år == min(år),region == region_val ,variabel == "Folkmängd") %>% .$varde,100),big.mark=" ") ` invånare  år `r min(befolkning_df$år)` till ungefär `r format(plyr::round_any(befolkning_df %>% filter(år == max(år),region == region_val, variabel == "Folkmängd") %>% .$varde,100),big.mark=" ") ` invånare år `r max(befolkning_df$år)`. Framförallt skedden en stor utflyttning under åren efter finanskrisen i början på 1990-talet. På senare år har dock utvecklingen sett annorlunda ut, då befolkningen i `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)`, med undantag för 2018 , har ökat varje år sedan 2014.

År `r max(befolkning_df$år)` ökade folkmängden med `r befolkning_df %>% filter(år == "2023",region == region_val, variabel == "Folkökning") %>% .$varde ` personer, vilket var en lägre ökning än åren innan. 
...
Bland de åldersgrupper som redovisas i diagrammet nedan skedde den största utflyttningen år `r max(flytt_aldersgrupper_df$år)` i gruppen som är under 20-29 år, vilket dock var en något lägre utflyttning (netto) än året innan. Bland barn och unga (0-19 år) var inflyttningen (netto) relativt stor `r max(flytt_aldersgrupper_df$år)`, medan det omvända gällde året innan.

```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.height=5, fig.width=8, fig.align='center'}
gg_flytt_alder$`Inrikes_flyttnetto_alder_Malung-Sälen`
```

Om fokus enbart läggs på åldersgruppen 20-29 år, blir det tydligt att det i normala fall flyttar fler unga vuxna från än till kommunen inrikes. Historiskt varierar det inrikes flyttnettot i åldersgruppen mellan åren, vilket exempelvis kan bero på att olika årskullar av barn är olika stora. År 2023 flyttade som `r abs(flytt_aldersgrupper_df %>% filter(år == max(år),variabel == "Inrikes flyttningsöverskott",region == region_val,alder_grupper == "20-29 år") %>% .$varde)` personer fler i åldersgruppen 20-29 år från än till kommunen inrikes, vilket var något färre än året innan. 
