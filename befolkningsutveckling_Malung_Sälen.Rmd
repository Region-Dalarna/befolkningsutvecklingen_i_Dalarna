---
title: "Befolkningsutvecklingen i Malung-Sälen"
author: ""
date: ""
output: 
  html_document:
    includes:
      in_header: hero_image_kommun.html
    toc: yes
    toc_float: yes
    toc_depth: 6
    css: "styles_hero_kommun.css"
    number_sections: true
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
region_val = "Malung-Sälen"
```


<p style="font-weight: 600;font-size: 1.5rem;line-height:1;padding-top:15px">Befolkningsutvecklingen i Malung-Sälen<br> <span style="font-weight: 400;font-size: 1.2rem">- en kort analys</span></p>

<style>
h1 { /* Header 1 */
  font-weight: 500;
  font-size: 1.3rem;
  color: #000000;
  margin-top: 2rem;
}
h2 { /* Header 2 */
  font-weight: 400;
  font-size: 1.1rem;
  color: #000000;
  margin-top: 1.5rem;
}
h3 { /* Header 3 */
  font-weight: 400;
  font-size: 1.1rem;
  color: #000000;
}
</style>

<p style = "font-size:12px">
<i>Rapporten är skapad av Samhällsanalys, Region Dalarna<br>
Senast uppdaterad: `r Sys.Date()`</i>
</p>

# Sammanfattning { - }
Syftet med den här korta analysen är att undersöka `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)`s befolkningsutveckling. Ett antal viktiga slutsatser sammanfattas här:

* Folkmängden i `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` minskade tydligt år 2025. Detta berodde i första hand på att det föddes ovanligt få barn i kommunen.
* En stramare nationell migrationspolitik gör att färre invandrar till Sverige än tidigare, vilket även påverkar inflyttningen till `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)`. Att invandringsöverskottet ändå ökat i `r region_val` på senare år kan sannolikt delvis förklaras med att personer födda i Ukraina sedan juni 2024 haft möjlighet att ansöka om att bli folkbokförda i Sverige.

För mer info om statistiken, klicka på rutan nedan:

<details>
<summary>Mer information om statistiken</summary>

<div title="Källa: SCB">I folkmängden som SCB publicerar ingår personer som är folkbokförda i landet. För att den som flyttar till Sverige ska bli folkbokförd ska personen ha för avsikt och rätt att stanna i Sverige i minst ett år. För personer som inte är medborgare i ett EU-land innebär det att det krävs ett uppehållstillstånd som gäller minst 12 månader för att bli folkbokförd. Till följd av Rysslands invasion av Ukraina den 24 februari 2022 har Europeiska unionen (EU) beslutat att aktivera massflyktsdirektivet, vilket innebär att personer från Ukraina utan individuell prövning ska erbjudas tidsbegränsat uppehålls- och arbetstillstånd inom EU. Personer som får uppehållstillstånd enligt massflyktsdirektivet ska i regel inte bli folkbokförda och ingå i den officiella befolkningsstatistiken. Läs mer [här](https://www.scb.se/hitta-statistik/statistik-efter-amne/befolkning-och-levnadsforhallanden/befolkningens-sammansattning-och-utveckling/befolkningsstatistik/produktrelaterat/Fordjupad-information/statistik-om-invandring---fragor-och-svar/){target="_blank"}. Sedan juni 2024 har dock personer födda i Ukraina haft möjlighet att ansöka om att bli folkbokförda i Sverige [(SCB)](https://www.scb.se/hitta-statistik/statistik-efter-amne/befolkning-och-levnadsforhallanden/befolkningens-sammansattning-och-utveckling/befolkningsstatistik/pong/statistiknyhet/befolkningsstatistik-helaret-20242/){target="_blank"}.

Under 2023 inledde Skatteverket ett arbete med att granska registret över folkbokförda. Det förde med sig att SCB avregistrerade flera tusen folkbokförda personer som visade sig inte längre bo i Sverige. I folkbokföringen har dessa registrerats som utvandrade till okänt utvandringsland. Detta arbete fortsatte under 2024. Läs mer [här](https://www.scb.se/pressmeddelande/sverige-gar-mot-lagsta-befolkningsokningen-pa-manga-ar/){target="_blank"}.

All data som används i rapporten är offentlig och finns under kategorin befolkning i SCB:s [Statistikdatabas](https://www.statistikdatabasen.scb.se/pxweb/sv/ssd/){target="_blank"}. Där finns även data för exempelvis befolkningsutvecklingen under 2025 på månadsbasis som publiceras löpande, något som inte behandlas i denna rapport.

Från och med referensåret 2025 och framåt är en liten kontrollerad slumpmässig osäkerhet införd i samtliga redovisade uppgifter. Osäkerheten har tillförts för att skydda enskilda personer så att ingen riskerar att röjas i statistiken. Till följd av detta så är inte alltid de redovisade totalerna lika med summan av deras redovisade delar.

Om redovisade statistikvärden adderas för att skapa en ny total, så adderas även osäkerheten. Det finns därför risk för att den nya totalen innehåller större osäkerhet än vad som, enligt metoden, skulle ha tillförts ett enskilt statistikvärde.

Metoden som används för röjandekontroll heter Cell Key Method (CKM). Du kan läsa mer om metoden här: [Skyddsmetoder för röjandekontroll](https://www.scb.se/dokumentation/statistiska-metoder/skyddsmetoder-for-rojandekontroll/){target="_blank"} och här: [utveckling av röjandeskyddet](https://www.scb.se/dokumentation/statistikomraden-under-forandring/utveckling-av-rojandeskyddet/){target="_blank"}, samt i befolkningsstatistikens dokumentation (kvalitetsdeklarationen, avsnitt 2.2.6).

FAQ om CKM-metoden hittar du här: [Frågor och svar om röjandeskydd i befolkningsstatistiken](https://www.scb.se/FAQ-CKM){target="_blank"}

</div>
</details>

# Hur har befolkningen förändrats?

I likhet med flera andra mindre kommuner i Dalarna har befolkningen i `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` minskat  sett över längre tid, från ca `r format(plyr::round_any(befolkning_df %>% filter(år == min(år),region == region_val ,variabel == "Folkmängd") %>% .$varde,100),big.mark=" ") ` invånare  år `r min(befolkning_df$år)` till ungefär `r format(plyr::round_any(befolkning_df %>% filter(år == max(år),region == region_val, variabel == "Folkmängd") %>% .$varde,100),big.mark=" ") ` invånare år `r max(befolkning_df$år)`. Framförallt minskade folkmängden mycket under åren efter finanskrisen i början på 1990-talet. På senare år har dock utvecklingen sett annorlunda ut, då befolkningen i `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)`, med undantag för 2018, ökade varje år mellan 2014 och 2023.

År `r max(befolkning_df$år)` `r ifelse(befolkning_df %>% filter(år == max(år),region == region_val, variabel == "Folkökning") %>% .$varde>0,"ökade","minskade")` dock folkmängden med `r abs(befolkning_df %>% filter(år == max(år),region == region_val, variabel == "Folkökning") %>% .$varde) ` personer.

```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.height=5, fig.width=8, fig.align='center'}
gg_befolkning$`Befolkningsutveckling_Malung-Sälen`
```

I fortsättningen av denna korta analys delas befolkningsutveckling upp i dess komponenter, födelsenetto, dvs. nettot av födda och döda, och flyttnetto, dvs. nettot av inrikes och utrikes flyttar.

# Födelsenetto

En trend är att det dör färre personer i `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` idag än det gjorde längre tillbaka i tiden. Normalt dör mellan 110 och 140 personer årligen i kommunen, att jämföra med närmare 200 personer under några år i början av 1990-talet. Detta kan delvis förklaras av att färre idag bor i kommunen, men den främsta anledningen är framsteg inom det medicinska området. Den förväntade livslängden har helt enkelt ökat över tid.

Även när det gäller antalet födslar är den långsiktiga trenden negativ. Under det sena 1980-talet och det tidigt 1990-talet föddes ofta närmare 150 barn i `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` per år, men en tydlig negativ trend inleddes i samband med finanskrisen i början på 1990-talet. Under 2000-talet har antalet födda legat på en relativt stabil nivå, även om variationen kan vara stor från år till år. `r max(fodda_doda_df$år)` föddes dock bara `r fodda_doda_df %>% filter(år == max(år),region == region_val,variabel == "födda") %>% .$varde` barn i kommunen, vilket var en kraftig minskning jämfört med åren innan.

Nettot mellan födda och döda, se de svarta linjerna i diagrammet nedan, har varit negativt i `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` under varje år sedan slutet på 1960-talet. Hur många invånare som kommunen tappar på grund av det varierar relativt mycket mellan åren. År `r max(fodda_doda_df$år)` medförde den låga nivån på barnafödandet att även födelsenettot minskade kraftigt jämfört med åren innan.

```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.height=5, fig.width=8, fig.align='center'}
gg_fodda_doda$`fodelsenetto_Malung-Sälen`
```

# Flyttnetto
Den andra komponenten som styr befolkningsutvecklingen är det så kallade flyttnettot. Detta kan i sin tur delas upp i två komponenter, inrikes och utrikes flyttar. I det här avsnittet är fokus först på flyttnettot, innan de två komponenterna undersöks.

Historiskt varierade nettot mellan antalet personer som flyttade till och från `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` relativt mycket mellan åren och var ofta negativt. Sedan mitten på 2010-talet har trenden dock förändrats och flyttnettot har varit positivt under varje år sedan 2015. Även  år `r max(flytt_df$år)` flyttade fler personer till än från kommunen, men nettot sjönk jämfört med åren innan.

```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.height=5, fig.width=8, fig.align='center'}
gg_flytt$`Flyttnetto_Malung-Sälen`

```

## Utrikes flyttnetto
Som berördes i föregående avsnitt, har flyttnettot till `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` varit positivt under varje år sedan 2015, vilket under framförallt 2016 och några år däromkring kan förklaras med att invandringen då var hög. I slutet av 1990-talet var invandringen låg eller till och med negativ under flera år (fler utvandrade än invandrade), men därefter skedde en gradvis ökning innan den stora toppen 2016. Därefter har invandringen minskat, men varit förhållandevis hög under vissa år. 

Efter att ha varit relativt låg 2023, ökade invandringen (netto) tydligt 2024 men även `r max(flytt_df$år)` var invandringen större än 2023. Detta kan sannolikt delvis förklaras med att personer födda i Ukraina sedan juni 2024 haft möjlighet att ansöka om att bli folkbokförda i Sverige (se [SCB](https://www.statistikdatabasen.scb.se/pxweb/sv/ssd/START__BE__BE0101__BE0101E/FolkmRegFlandK/table/tableViewLayout1/){target="_blank"} och [SCB](https://www.statistikdatabasen.scb.se/pxweb/sv/ssd/START__BE__BE0101__BE0101E/FodelselandArKCKM/){target="_blank"}). Läs mer i den klickbara rutan "mer information om statistiken" ovan.

Värt att notera är även att Skattemyndigheten under 2023 påbörjade en avregistrering av utvandrade, vilket kan få viss påverkan på utvandringen under åtminstone 2023 och 2024 (se informationsrutan under avsnittet introduktion ovan). Den viktigaste anledningen till att invandringen generellt är lägre idag än åren från mitten av 2010-talet är dock sannolikt en stramare nationell migrationspolitik.

Generellt invandrar ungefär lika många kvinnor och män till `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)`.

```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.height=5, fig.width=8, fig.align='center'}
gg_flytt$`Invandringsöverskott_Malung-Sälen`
```

## Inrikes flyttnetto
Det inrikes flyttnettot till `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` varierar relativt mycket mellan åren, men har varit negativt under en majoritet av åren sedan slutet på 1990-talet. På senare år har dock fler personer flyttat till än från `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` inrikes vid flera tillfällen, då framförallt under åren runt Corona-pandemin (2020 och 2021). År `r max(befolkning_df$år)` var det inrikes flyttnettot dock återigen negativt, precis som året innan.

```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.height=5, fig.width=8, fig.align='center'}
gg_flytt$`Inrikes flyttningsöverskott_Malung-Sälen`
```

I nästa avsnitt är fokus istället på framtiden. Hur kommer `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)`s folkmängd att utvecklas fram till 2034?

# Vad händer i framtiden?

*Notera att prognosen nedan gjordes våren 2025. Avsnittet kommer därför att uppdateras med en ny prognos under våren 2026.*

För en djupare förståelse runt hur Region Dalarna gör befolkningsprognoser, se motsvarande avsnitt i rapporten för hela Dalarnas län. Det viktigaste att ha i åtanke är att ingen hänsyn tas i prognoserna till exempelvis företagsetableringar eller andra framtida mål och förutsättningar som kommunerna har i sina planer.

I diagrammet nedan visas befolkningsutvecklingen i `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` fram till `r prognos_ar` uppdelat på åldersgrupper, enligt Region Dalarnas prognos från våren 2025.

* Folkmängden i `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` prognostiseras att minska till `r befprognos_Malung_totalt` personer år `r prognos_ar`, en minskning med ungefär `r befprognos_Malung_totalt_forandring` personer jämfört med `r prognos_forsta_ar`. I SCB:s senaste prognos (juni 2024), prognostiserades en befolkning på knappt 10 120 personer i `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` år `r prognos_ar` (se [SCB, 2024](https://www.statistikdatabasen.scb.se/pxweb/sv/ssd/START__BE__BE0401__BE0401A/BefProgRegFakN/){target="_blank"}).

* Den största minskningen sker i åldersgruppen 0-19 år, som prognostiseras minska med ungefär `r befprognos_Malung_0_19_forandring` personer.

* I åldergruppen 80+ förväntas befolkning istället att öka med cirka `r befprognos_Malung_80plus_forandring` personer fram till `r prognos_ar`. Detta hänger sannolikt samman med att den förväntade livslängden fortsätter öka och att den stora gruppen "baby-boomers" börjar komma upp i 80-års åldern.

```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.height=5, fig.width=8, fig.align='center'}
gg_befprognos$`befprogn_Malung-Sälen_2025_10ars_sikt_antal`
```
