...
Bland de åldersgrupper som redovisas i diagrammet nedan skedde den största utflyttningen år `r max(flytt_aldersgrupper_df$år)` i gruppen som är under 20 år. Utflyttningen (netto) i den gruppen minskade dock tydligt jämfört med året innan.

Den största inflyttningen, netto, skedde i åldersgruppen `r flytt_aldersgrupper_df %>% filter(år == max(år),variabel == "Inrikes flyttningsöverskott",region == region_val) %>% filter(varde==max(varde)) %>% .$alder_grupper`. Även i den gruppen var förändringen jämfört med föregående år positiv, då flyttnettot var negativt.
