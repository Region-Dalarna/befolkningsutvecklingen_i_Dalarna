...
Nettot mellan antalet personer som flyttar till och från `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` varierar relativt mycket mellan åren, men har i perioder varit positivt. Framförallt flyttade många till kommunen mellan 2014 och 2016, men även under andra hälften av 00-talet var det en relativt stor inflyttning. Efter några år med negativt flyttnetto var det återigen positivt under `r max(flytt_df$år)`, då `r flytt_df %>% filter(år==max(år),region == region_val) %>% group_by(region) %>% summarize(varde=sum(varde)) %>% .$varde` personer flyttade till kommunen netto.
