...
När gäller antalet födslar är den långsiktiga trenden något negativ, det föddes något fler barn i Dalarna på slutet av 1960-talet än det gör idag. Under tidigt 1990-tal föddes ofta över 150 barn i `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` per år, men en tydlig negativ trend inleddes i samband med finanskrisen i början på 1990-talet, när befolkningen också minskade kraftigt.Under 2010-talet ökade födslarna igen, något som sannolikt har att göra med att `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)`s befolkning då ökade till följd av högre invandring (mer om det senare). År `r max(fodda_doda_df$år)` föddes `r fodda_doda_df %>% filter(år == max(år),region == region_val,variabel == "födda") %>% .$varde` barn, vilket är den lägsta siffran sedan 2007.
...
Sammanfattningsvis minskade `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` folkmängd under `r max(befolkning_df$år)` med `r abs(befolkning_df %>% filter(år == max(år),region == "Falun",variabel == "Folkökning") %>% .$varde)` personer, vilket i första hand beror på att det föds relativt få barn (födelsenettot är negativt).
