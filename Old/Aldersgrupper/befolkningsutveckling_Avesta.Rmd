...
År `r max(flytt_df$år)` var det inrikes flyttnettot drygt `r plyr::round_any(flytt_df %>% filter(år == max(år),region == region_val, variabel == "Inrikes flyttningsöverskott") %>% group_by(region) %>% summarize(varde=sum(varde)) %>% .$varde,10)` personer, det största inrikes flyttnettot sedan 2015. Detta var en tydlig förändring jämfört med året innan, när nettot var negativt (drygt `r plyr::round_any(abs(flytt_df %>% filter(år == (as.numeric(max(flytt_df$år))-1),region == region_val, variabel == "Inrikes flyttningsöverskott") %>% group_by(region) %>% summarize(varde=sum(varde)) %>% .$varde),10)` personer). Detta är en viktig förklarande faktor till varför befolkningen i `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` inte minskade lika mycket `r max(flytt_df$år)`, som åren innan.
