...
När det gäller antalet födslar är trenden negativ om man jämför med framförallt det tidiga 1990-talet, men under 2000-talet har antalet födda legat på en relativt stabil nivå. Under slutet av 1980-talet föddes vissa år över 200 barn i `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)`, att jämföra med strax över `r plyr::round_any(fodda_doda_df %>% filter(år == max(år),region == region_val,variabel == "födda") %>% .$varde,100)` barn år `r max(fodda_doda_df$år)`.


Nettot mellan födda och döda, se de svarta linjerna i diagrammet nedan, varierar relativt mycket mellan åren, men är i praktiken alltid negativt i Leksand, en indikation på att kommunen har en relativt hög medelålder.
