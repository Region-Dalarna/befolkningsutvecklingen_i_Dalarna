...
* Utflyttningen (netto) i åldersgruppen 0-19 år minskade tydligt under 2025, jämfört med åren innan.
...
### Uppdelat på ålder
Ett tydligt mönster på senare år är att det i första hand är personer under 40 år som lämnar `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` för andra delar av Sverige, medan flyttnetto är mer otydligt i övriga åldersgrupper. Under såväl 2024 som `r max(flytt_aldersgrupper_df$år)` var dock det inrikes flyttnettot negativt i samtliga åldersgrupper som redovisas i diagrammet nedan.

Den största utflyttningen år `r max(flytt_aldersgrupper_df$år)` skedde åldersgruppen 30 - 39 år (`r sum(flytt_aldersgrupper_df %>% filter(år == max(år),variabel == "Inrikes flyttningsöverskott",region == region_val,alder_grupper%in%c("30-39 år")) %>% .$varde)` personer). Även bland personer under 30 år, var antalet uflyttare fler än antalet inflyttare (`r sum(flytt_aldersgrupper_df %>% filter(år == max(år),variabel == "Inrikes flyttningsöverskott",region == region_val,alder_grupper%in%c("0-19 år","20-29 år")) %>% .$varde)` personer). Intressant att notera är dock att det inrikes flyttnettot i åldersgruppen 0-19 år var klart mindre negativt år `r max(flytt_aldersgrupper_df$år)`, jämfört med året innan.

```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.height=5, fig.width=8, fig.align='center'}
gg_flytt_alder$Inrikes_flyttnetto_alder_Borlänge
```

Om fokus enbart läggs på åldersgruppen 20-29 år, blir det tydligt att det under de allra flesta år flyttar fler unga vuxna från än till kommunen inrikes. Historiskt varierar det inrikes flyttnettot i åldersgruppen mellan åren, vilket exempelvis kan bero på att olika årskullar av barn är olika stora. Under `r max(flytt_aldersgrupper_df$år)` flyttade `r abs(flytt_aldersgrupper_df %>% filter(år == max(år),variabel == "Inrikes flyttningsöverskott",region == region_val,alder_grupper == "20-29 år") %>% .$varde)` personer netto från kommunen inrikes, vilket var en större utflyttning än året innan men en lägre än åren dessförinnan.
