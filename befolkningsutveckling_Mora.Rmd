...
Sammanfattningsvis minskade `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` folkmängd under `r max(befolkning_df$år)` med `r abs(befolkning_df %>% filter(år == max(år),region == "Mora",variabel == "Folkökning") %>% .$varde)` personer. Detta hänger med att alla komponenter som förklarar befolkningsutvecklingen, födelsenetto samt inrikes och utrikes flyttnetto, försämrades under 2023 jämfört med åren innan.
