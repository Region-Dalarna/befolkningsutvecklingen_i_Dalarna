...
Nettot mellan antalet personer som flyttar till och från `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)`, som varierar relativt mycket mellan åren, har varit positivt under stora delar av 2010-talet. Många flyttade till kommunen i mitten på 2010-talet, men även under 2020 och 2021 var inflyttningen stor (netto). År `r max(flytt_df$år)` var flyttnettot dock tydligt negativt (`r flytt_df %>% filter(år==max(år),region == region_val) %>% group_by(region) %>% summarize(varde=sum(varde)) %>% .$varde` personer). Detta var det lägsta nettot sedan år 2000.
