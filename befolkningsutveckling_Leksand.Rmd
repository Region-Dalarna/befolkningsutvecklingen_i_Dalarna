...
Sammanfattningsvis minskade `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` folkmängd under `r max(befolkning_df$år)` med `r abs(befolkning_df %>% filter(år == max(år),region == "Leksand",variabel == "Folkökning") %>% .$varde)` personer. Detta hänger samman med att såväl det inrikes som det utrikes flyttnettot var negativt.
