...
* `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` befolkning prognostiseras att minska till `r befprognos_Leksand_totalt` personer år `r prognos_ar`, en minskning med ungefär `r befprognos_Leksand_totalt_forandring` personer jämfört med `r prognos_forsta_ar`. I SCB:s senaste prognos (juni 2024), prognostiserades en befolkning på 16 240 personer i `r unique(befolkning_df %>% filter(region == region_val) %>% .$region)` år `r prognos_ar` (se [SCB, 2024](https://www.statistikdatabasen.scb.se/pxweb/sv/ssd/START__BE__BE0401__BE0401A/BefProgRegFakN/){target="_blank"}).

* Den största minskningen sker i åldersgruppen 0-19 år, som prognostiseras minska med ungefär `r befprognos_Leksand_0_19_forandring` personer.

* I åldergruppen 80+ förväntas befolkning istället att öka med ungefär `r befprognos_Leksand_80plus_forandring` personer fram till `r prognos_ar`. Detta hänger sannolikt samman med att den förväntade livslängden fortsätter öka och att den stora gruppen "baby-boomers" börjar komma upp i 80-års åldern.
